...
def tfrec_to_bytes():
    ...
    for i in cfg.train_files_224:
        ...
        ids = [
            str(id_features["id"].numpy())[2:-1] for id_features in train_image_dataset
        ]  # [2:-1] is done to remove b' from 1st and 'from last in train id names
        train_ids = train_ids + ids
        classes = [
            int(class_features["class"].numpy())
            for class_features in train_image_dataset
        ]
        train_class = train_class + classes
        images = [
            image_features["image"].numpy() for image_features in train_image_dataset
        ]
    ...
    for i in cfg.val_files_224:
        ...
        ids = [
            str(id_features["id"].numpy())[2:-1] for id_features in val_image_dataset
        ]  # [2:-1] is done to remove b' from 1st and 'from last in train id names
        val_ids = val_ids + ids
        classes = [
            int(class_features["class"].numpy()) for class_features in val_image_dataset
        ]
        val_class = val_class + classes
        images = [
            image_features["image"].numpy() for image_features in val_image_dataset
        ]
